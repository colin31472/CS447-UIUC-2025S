...
Original file is located at
    https://colab.research.google.com/drive/1Nx1dheaT2qyQJzNqY6aiCPI928021d5j
...
class RnnEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        ### TODO ###
        output, hidden_state = self.gru(self.embedding(x))
...
class RnnDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, hidden_units):
        ...
        self.fcl = nn.Linear(hidden_units, vocab_size)
    ...
    def compute_attention(self, dec_hs, enc_output):
        ...
        dec_hs = dec_hs.squeeze(0).unsqueeze(1)

        scores = self.v_a(torch.tanh(self.w1(dec_hs) + self.w2(enc_output)))
    ...
    def forward(self, x, dec_hs, enc_output):
        ...
        concated = torch.cat((self.embedding(x), context_vector.unsqueeze(1)), dim=2)

        gru_out, dec_hs = self.gru(concated, dec_hs)
        fc_out = self.fcl(gru_out.squeeze(1))
...
def sample_model(encoder, decoder, src, max_decode_len, temperature):
    ...
    device = src.device
    curr_output = torch.zeros((batch_size, max_decode_len), dtype=torch.long, device=device)
    ...
    for t in range(1, max_decode_len):
        if is_rnn:
            ...
            fc_out, dec_hiddden = decoder.decode_step(curr_output[:, :t].long(), enc_output, dec_hidden)

        distributions = torch.distributions.Categorical(logits=fc_out / temperature)
        distributions = torch.distributions.Categorical(logits=fc_out/temperature)

        next_tokens = distributions.sample()
        curr_output[:, t] = next_tokens
        dec_input = next_tokens.unsqueeze(1)

    curr_output = curr_output.cpu().numpy()

    ### TODO ###
    #   For each item in the batch, truncate the sentence to EOS; if EOS was not predicted, choose sentence up to max_decode_len
    for sample in curr_output:
        sample_list = sample.tolist()
        if eos_idx in sample_list:
            sentence = sample_list[:sample_list.index(eos_idx)+1]
        else:
            sentence = sample_list
...
def sort_predictions(predictions):
    ...
    ### TODO ###
    probs, sorted = torch.sort(predictions.contiguous().view(-1), descending=True)
    hypothesis_ids = sorted // trg_vocab_size
    idxes = sorted % trg_vocab_size
...
def find_eos_sentences(probs, idxes, hypothesis_ids, curr_output, eos_idx, beam_size, alpha):
    ...
    #         with the EOS token.
    valid_positions = torch.nonzero((idxes == eos_idx) & (torch.arange(probs.size(0), device=probs.device) < beam_size), as_tuple=False).squeeze(-1)
    # if valid_positions.numel() == 0:
    #     return eos_sents, eos_probs

    lp = ((5 + curr_output.size(1) - 1) / 6) ** alpha

    for pos in valid_positions:
        score = probs[pos].item() / lp
        sentence = curr_output[hypothesis_ids[pos].item()].tolist() + [eos_idx]

        eos_probs.append(score)
        eos_sents.append(sentence)
...
def find_non_eos_sentences(probs, idxes, hypothesis_ids, curr_output, eos_idx, beam_size, t):
    ...
    valid_positions = torch.nonzero((idxes != eos_idx), as_tuple=False).squeeze(-1)

    beam_size_valid = valid_positions[:beam_size]

    new_probs = probs[beam_size_valid]
    next_hyps = hypothesis_ids[beam_size_valid]

    for i in range(beam_size):
        hyp_idx = next_hyps[i].item()
        new_output[i, :t] = curr_output[hyp_idx, :t]
        new_output[i, t] = idxes[beam_size_valid][i]
...
def beam_decode_model(encoder, decoder, src, max_decode_len, beam_size,alpha):
    ...
    probs, idxes, hypothesis_ids = sort_predictions(torch.log_softmax(fc_out, dim=-1))
    curr_output, curr_probs, hyps = find_non_eos_sentences(
        probs, idxes, hypothesis_ids, curr_output, eos_idx, beam_size, t=1
    )

    if is_rnn: dec_hidden = dec_hidden.repeat(1, beam_size, 1)
    ...
    for t in range(2, max_decode_len):
        fc_out, dec_hidden = decoder.decode_step(curr_output[:, t-1].long().unsqueeze(1), enc_output, dec_hidden)
        cumulative_log_probs = curr_probs.unsqueeze(1) + torch.log_softmax(fc_out, dim=-1)

        probs, idxes, hypothesis_ids = sort_predictions(cumulative_log_probs)

        eos_sents, eos_scores = find_eos_sentences(
            probs, idxes, hypothesis_ids, curr_output, eos_idx, beam_size, alpha
        )
        ...
        curr_output, curr_probs, hyps = find_non_eos_sentences(
            probs, idxes, hypothesis_ids, curr_output, eos_idx, beam_size, t
        )

        if is_rnn: dec_hidden = dec_hidden[:, hyps.long(), :]
    ...
    if len(completed_scores) > 0:
        sentence = completed_hypotheses[torch.tensor(completed_scores).argmax().item()]
    else:
        sentence = curr_output[torch.argmax(curr_probs).item()].tolist()
    ...
    if eos_idx in sentence:
        ...
        while sentence and sentence[-1] == 0:
            sentence.pop()
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        ### TODO ###
        x = x.to(self.device)
        embedded = self.dropout(self.embedding(x) + self.positional_embedding[:x.size(0)])
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, dec_in, enc_out):
        ...
        embedded = self.dropout(self.embedding(dec_in) + self.positional_embedding[:dec_in.size(0)])
        trg_mask = self.generate_square_subsequent_mask(dec_in.size(0))
        decoder_output = self.transformer_decoder(tgt=embedded, memory=enc_out, tgt_mask=trg_mask)
