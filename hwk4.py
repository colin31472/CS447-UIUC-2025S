...
Original file is located at
    https://colab.research.google.com/drive/1poFx8c8_1UCU0bGGECeFEe3DYx3_RJ6L
...
!pip install cloudpickle numpy==1.26.4 spacy torch tqdm ipywidgets
...
class ParserConfiguration(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            w_j = self.stack.pop()
            w_i = self.stack.pop()
            self.dependencies.add(w_j, w_i, transition[3:])
            self.stack.push(w_j)
        elif transition.startswith('RA-'):
            w_j = self.stack.pop()
            w_i = self.stack.get_si(1)
            self.dependencies.add(w_i, w_j, transition[3:])
        else:
            raise ValueError(f"Unknown transition")
...
def get_gold_action(stack, buffer, gold_dependencies):
    ...
    if len(stack) == 1 and len(buffer) == 0:
        ...
        s_1 = stack.get_si(1)
        s_2 = stack.get_si(2)
        arc_s2 = gold_dependencies.getArcToHead(s_2)
        if arc_s2.head == s_1:
            action = 'LA-' + arc_s2.label
        else:
            arc_s1 = gold_dependencies.getArcToHead(s_1)
            if arc_s1.head == s_2:
                can_ra = True
                for i in range(1, len(buffer)+1):
                    b = buffer.get_bi(i)
                    if gold_dependencies.getArcToHead(b).head == s_1:
                        can_ra = False
                        break
                if can_ra:
                    action = 'RA-' + arc_s1.label
...
def get_top3_stack_features(parser_config):
    ...
    for i in range(1,4):
        if parser_config.stack.get_si(i) is not None:
            word_features[i-1] = parser_config.stack.get_si(i).word_id
            pos_features[i-1] = parser_config.stack.get_si(i).pos_id
...
def get_top3_buffer_features(parser_config):
    ...
    for i in range(1,4):
        if parser_config.buffer.get_bi(i) is not None:
            word_features[i-1] = parser_config.buffer.get_bi(i).word_id
            pos_features[i-1] = parser_config.buffer.get_bi(i).pos_id
...
def get_lc(parser_config, word):
    ...
    '''
    ### TODO ###
    left_arcs = [arc for arc in parser_config.dependencies if arc.head == word and arc.dependent.idx < word.idx]
    left_arcs.sort(key=lambda arc: arc.dependent.idx)
...
def get_rc(parser_config, word):
    ...
    '''
    ### TODO ###
    right_arcs = [arc for arc in parser_config.dependencies if arc.head == word and arc.dependent.idx > word.idx]
    right_arcs.sort(key=lambda arc: arc.dependent.idx, reverse=True)
...
def get_lc1_lc2_features(parser_config, i):
    ...
    if parser_config.stack.get_si(i) is not None:
        ...
        for j in range(min(2, len(lc_arcs))):
            word_features[j] = lc_arcs[j].dependent.word_id
            pos_features[j] = lc_arcs[j].dependent.pos_id
            label_features[j] = lc_arcs[j].label_id
...
def get_rc1_rc2_features(parser_config, i):
    ...
    if parser_config.stack.get_si(i) is not None:
        ...
        for j in range(min(2, len(rc_arcs))):
            word_features[j] = rc_arcs[j].dependent.word_id
            pos_features[j] = rc_arcs[j].dependent.pos_id
            label_features[j] = rc_arcs[j].label_id
...
def get_llc_rrc_features(parser_config, i):
    ...
    if parser_config.stack.get_si(i) is not None:
        lc_arcs = get_lc(parser_config, parser_config.stack.get_si(i))
        if len(lc_arcs) > 0:
            lc1 = lc_arcs[0].dependent
            llc_arcs = get_lc(parser_config, lc1)
            if len(llc_arcs) > 0:
                word_features[0] = llc_arcs[0].dependent.word_id
                pos_features[0] = llc_arcs[0].dependent.pos_id
                label_features[0] = llc_arcs[0].label_id

        rc_arcs = get_rc(parser_config, parser_config.stack.get_si(i))
        if len(rc_arcs) > 0:
            rc1 = rc_arcs[0].dependent
            rrc_arcs = get_rc(parser_config, rc1)
            if len(rrc_arcs) > 0:
                word_features[1] = rrc_arcs[0].dependent.word_id
                pos_features[1] = rrc_arcs[0].dependent.pos_id
                label_features[1] = rrc_arcs[0].label_id
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden = F.relu(self.hidden(embedding))
        dropout = self.dropout(hidden)
        logits = self.output(dropout)
...
def select_best_legal_action(parser_configs, predictions, n_labels):
    ...
    ### TODO ###
    preds = np.zeros(predictions.shape[0], dtype=int)

    for i, pc in enumerate(parser_configs):
        scores = predictions[i]
        legal = np.zeros(scores.shape, dtype=bool)
        if len(pc.buffer) >= 1:
            legal[2 * n_labels] = True
        if len(pc.stack) >= 2:
            if pc.stack.get_si(2).idx != 0 and pc.stack.get_si(2).idx not in pc.dependencies.dep_to_head_mapping:
                legal[:2 * n_labels] = True
            if pc.stack.get_si(1).idx not in pc.dependencies.dep_to_head_mapping:
                legal[n_labels:2 * n_labels] = True

        preds[i] = int(np.argmax(np.where(legal, scores, -np.inf)))
...
To submit the assignment, download this notebook as a <TT>.py</TT> file. You can do this by going to <TT>File > Save and Export Notebook As... > Executable Script</TT>. Then rename it to `hwk4.py`. <b>Do not try to submit it as a <TT>.ipynb</TT> file!</b>

You will also need to save your trained `model`. You can run the cell below to do this. After you save the files, you need to manually download the file to your computer, and then submit them to the autograder.
