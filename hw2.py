...
Original file is located at
    https://colab.research.google.com/drive/1Em8MMTOboxCy64cwegQjM9--j7WVpbRi
...
class CbowDataset(data.Dataset):
    ...
    def make_training_examples(self):
        ...
        self.training_examples += [(sentence[i-self.context_size:i] + sentence[i+1:i+self.context_size+1], sentence[i]) for sentence in self.sentences for i in range(self.context_size, len(sentence)-self.context_size)]
    ...
    def get_target_index(self, idx):
        ...
        return self.word2idx[self.training_examples[idx][1]]
...
class TextDataset(data.Dataset):
    ...
    def build_dictionary(self):
        ...
        word_freq = {}
        for label, words in self.examples:
            for word in words:
                word = word.lower()
        ...
        for word in sorted_words:
            if word_freq[word] >= self.threshold:
        ...
        for label, words in self.examples:
            ...
            for word in words:
                ...
                if word in self.word2idx:
                    review_idx.append(self.word2idx[word])
                else:
                    review_idx.append(self.word2idx[CNN_UNK])
            review_idx.append(self.word2idx[CNN_END])
            self.textual_ids.append(review_idx)
    ...
    def get_text(self, idx):
        ...
        if len(review) > self.max_len:
            modified_review = review[:self.max_len]
        else:
            modified_review = review + [self.word2idx[CNN_PAD]] * (self.max_len - len(review))

        return torch.LongTensor(modified_review)
...
class CNN(nn.Module):
    ...
    def forward(self, texts):
        ...
        embedded = self.embedding(texts).permute(0,2,1)
        ...
        for convolution in self.convolutions:
            ...
            output = F.max_pool1d(output, kernel_size=output.shape[2]).squeeze(2)
            outputs.append(output)
        concated = torch.cat(outputs, dim=1)
