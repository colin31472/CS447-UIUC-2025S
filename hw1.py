...
Original file is located at
    https://colab.research.google.com/drive/1S9sE8qSB7SqUksxEJyEane6CmmkKv2S5
...
class WordPieceTokenizer(object):
    ...
    def initialize(self, train_corpus):
          ...
          for word in words:
            ...
            for i in range(len(word)):
              if i == 0:
                ...
                vocab.add('##'+word[i])
                tokenized.append('##'+word[i])
            word_tokenizations[word] = tuple(tokenized)
        ...
        for token in vocab:
            words_with_token = []
            for word, used_tokens in word_tokenizations.items():
              if token in used_tokens:
                words_with_token.append(word)
            tokens2word[token] = set(words_with_token)
        del tokens2word[' ']
        del tokens2word['<UNK>']
    ...
    def find_best_pair(self, word_tokenizations, word_freqs, vocab):
        ...
        for pair, count in pair_counts.items():
            new_token = merge_token(pair)
            if new_token in vocab:
                continue
            a, b = pair
            scores[pair] = (count * len(vocab)) / (token_counts[a] * token_counts[b])

        if scores:
            best_pair = min(sorted(scores.keys()), key=lambda pair: (-scores[pair], pair))
        else:
            best_pair = None
    ...
    def merge_best_pair(self, best_pair, vocab, word_tokenizations, tokens2word):
        ...
        new_token = "##" + new_token if "##" in best_pair[0] else new_token
        vocab.add(new_token)

        words_to_update = tokens2word.get(best_pair[0], set()).union(tokens2word.get(best_pair[1], set()))
        ...
        for word in words_to_update:
          if new_token in word_tokenizations[word]:
    ...
    def tokenize(self, sentence):
        ...
        for i, word in enumerate(words):
          ...
          while word:
            ...
            if longest_substr is None:
              sub_tokens = ["<UNK>"]
              break

            sub_tokens.append(longest_substr)
            word = word[len(longest_substr):]
...
class UnigramModel(LanguageModel):
    def __init__(self, trainCorpus):
        ...
        self.probs = {word: self.word_freq[word] / self.total_count for word in self.word_freq}
    ...
    def generateSentence(self):
        ...
        sentence = ["<s>"]
        while True:
            next_word = random.choices(list(self.probs.keys()), weights=list(self.probs.values()))[0]
    ...
    def getSentenceLogProbability(self, sentence):
        ...
        for word in sentence:
            if word == "<s>":
                continue
            log_prob += math.log(self.probs[word])
    ...
    def getCorpusPerplexity(self, testCorpus):
        ...
        for sentence in testCorpus:
            tot_log_prob += self.getSentenceLogProbability(sentence)
            tot_word_count += len(sentence) - 1

        if tot_word_count == 0:
            return float('inf')

        perplexity = math.exp(-tot_log_prob / tot_word_count)
...
class SmoothedUnigramModel(UnigramModel):
    def __init__(self, trainCorpus):
        ...
        self.probs = {
            word: (self.word_freq[word] + 1) / (self.total_count + self.vocab_size)
...
class BigramModel(LanguageModel):
    def __init__(self, trainCorpus):
        ...
        for sentence in trainCorpus:
            for prev, cur in zip(sentence, sentence[1:]):
                if prev not in self.bigram_count:
                    self.bigram_count[prev] = {}
                    self.bigram_total[prev] = 0
                self.bigram_count[prev][cur] = self.bigram_count[prev].get(cur, 0) + 1
                self.bigram_total[prev] += 1

        self.bigram_probs = {}
        self.bigram_choices = {}
        for prev in self.bigram_count:
            self.bigram_probs[prev] = {}
            words = []
            weights = []
            for cur, count in self.bigram_count[prev].items():
                self.bigram_probs[prev][cur] = count / self.bigram_total[prev]
                words.append(cur)
                weights.append(count / self.bigram_total[prev])
            self.bigram_choices[prev] = (words, weights)
    ...
    def generateSentence(self):
        ...
        while True:
            prev = sentence[-1]
            words, weights = self.bigram_choices[prev]
    ...
    def getSentenceLogProbability(self, sentence):
        ...
        log_prob = 0.0
        for prev, cur in zip(sentence, sentence[1:]):
            if prev not in self.bigram_probs or cur not in self.bigram_probs[prev]:
                return float('-inf')
            log_prob += math.log(self.bigram_probs[prev][cur])
...
class SmoothedBigramModelAD(BigramModel):
    def __init__(self, trainCorpus):
        ...
        self.S = {}
        for prev in self.bigram_count:
            self.S[prev] = len(self.bigram_count[prev])
        ...
        self.vocab = list(self.unigram_count.keys())

        self.laplace_probs = {}
        for word in self.vocab:
            self.laplace_probs[word] = (self.unigram_count[word] + 1) / (self.total_unigram + len(self.vocab))
        ...
        for prev in self.bigram_count:
            ...
            weights = []
            for cur, count in self.bigram_count[prev].items():
                prob = max(count - self.D, 0) / self.bigram_total[prev] + (self.D * self.S[prev] / self.bigram_total[prev]) * self.laplace_probs[cur]
                self.bigram_probs[prev][cur] = prob
                words.append(cur)
                weights.append(prob)
            self.bigram_choices[prev] = (words, weights)
    ...
    def getSentenceLogProbability(self, sentence):
        log_prob = 0.0
        for prev, cur in zip(sentence, sentence[1:]):
            if cur in self.bigram_probs[prev]:
                prob = self.bigram_probs[prev][cur]
            else:
                prob = (self.D * self.S[prev] / self.bigram_total[prev]) * self.laplace_probs[cur]
